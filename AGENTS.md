...
pnpm --filter @zenml/shared-kitaru generate:types -- <base-url>
```
...
- `../../shared/kitaru/src/api/openapi.d.ts` — generated OpenAPI types (do not hand-edit)
...
- `../../shared/kitaru/src/api/openapi.d.ts` — generated by `pnpm --filter @zenml/shared-kitaru generate:types -- <base-url>` from the ZenML OpenAPI spec
...
- Keep comments minimal — code should be self-explanatory. Don't add comments (incl. JSDoc) that restate a name or type (e.g. a `/** ... */` on a well-named field/prop). Comment only genuinely non-obvious "why" (tricky edge cases, surprising decisions), and keep those lean.
...
- Factory functions for API fixture data live in `e2e/fixtures/api/` and are typed against `../../shared/kitaru/src/api/openapi.d.ts` — TypeScript catches schema drift at compile time
