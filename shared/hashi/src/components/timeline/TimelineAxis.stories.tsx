...
// the active spans keep room to breathe.
const COMPRESSED_MAP = buildTimeMap(TIMELINE_NODES, EXTENT_MS, TRACK_PX);
// Linear scale maps time proportionally end to end — the gap eats most of the
// width, which is exactly what the compressed mode exists to avoid.
const LINEAR_MAP = buildTimeMap(
	TIMELINE_NODES,
	EXTENT_MS,
	TRACK_PX,
	undefined,
...
// ─── Linear scale ─────────────────────────────────────────────────────────────
// Time maps proportionally, so the idle gap dominates and the active spans get
// squeezed. Ticks anchor `0s` and the exact end.

export const LinearScale: Story = {
	render: (args) => (
		<AxisFrame label="Linear scale — gap eats the width">
			<TimelineAxis {...args} timeMap={LINEAR_MAP} />
		</AxisFrame>
	),
};
