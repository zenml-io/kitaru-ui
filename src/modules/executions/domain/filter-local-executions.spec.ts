...
describe("filterLocalExecutions", () => {
	it("keeps runs whose snapshot isn't a known Kitaru deployment", () => {
		const e1 = mkExecution({ id: "run-1", sourceSnapshotId: "snap-a" });
		const e2 = mkExecution({ id: "run-2", sourceSnapshotId: "snap-kitaru" });
		const e3 = mkExecution({ id: "run-3", sourceSnapshotId: undefined });
	...
	it("treats a run with no snapshotId as local", () => {
		const e = mkExecution({ id: "run-1", sourceSnapshotId: undefined });
